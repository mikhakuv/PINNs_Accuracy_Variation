...
Этот момент уточнён для того, чтобы обосновать изучение производных действительной и мнимой частей уравнения по отдельности, ведь при оптимизации они находятся отдельно. На графике 6 видно, как увеличиваются все слагаемые минимизируемых выражений при росте $\mu$ при $t=0$:  
...
При других $t$ наблюдается аналогичная картина.(?) А в приведённой ниже таблице рассчитано, во сколько раз увеличивается каждое из слагаемых. Видно, что наибольший рост испытывает слагаемое $v*(u^2+v^2)(1-\alpha(u^2+v^2)+\beta(u^2+v^2)^2)$, которое при используемых в данной работе значениях коэффициентов принимает вид: $v*(u^2+v^2)$. Рост более, чем пятикратный. Вероятно, что это также осложняет процесс обучения и точность решения также снижается.
