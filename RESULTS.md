...
$Re(F(q,q_{t},q_{xx}))=-v_t+u_{xx}+u*(u^2+v^2)(1-\alpha(u^2+v^2)+\beta(u^2+v^2)^2)$  
$Im(F(q,q_{t},q_{xx}))=u_t+v_{xx}+v*(u^2+v^2)(1-\alpha(u^2+v^2)+\beta(u^2+v^2)^2)$  
Этот момент уточнён для того, чтобы обосновать изучение производных действительной и мнимой частей функции по отдельности, ведь при оптимизации они находятся отдельно. На графике 6 видно, как увеличиваются абсолютные значения всех слагаемых в обоих минимизируемых выражениях $Re(F)$ и $Im(F)$ при росте $\mu$ и фиксированном $t=0$:  
...
В приведённой ниже таблице рассчитано максимальное значение модуля каждого из слагаемых при $t=0$(при других $t$ картина похожая) и его относительный рост в столбце **relation**:  
...
\end{table}
-->  
Видно, что наибольший рост испытывает слагаемое $v*(u^2+v^2)(1-\alpha(u^2+v^2)+\beta(u^2+v^2)^2)$, которое при используемых в данной работе значениях коэффициентов принимает вид: $v*(u^2+v^2)$. Рост более чем пятикратный, остальные слагаемые тоже заметно увеличиваются и это влияет на сложность задачи оптимизации.  
Также можно оценить нелинейность уравнения, которая определяется здесь как отношение модуля нелинейного слагаемого к модулю всех остальных слагаемых: $\frac{||q|^2 q|}{|iq_t+q_{xx}|}$  
|                             | mu=2.30 | mu=4.15 | mu=6.50  |
|-----------------------------|---------|---------|----------|
| \|iq_t + q_xx\|             | 20.521  | 45.121  | 82.383   |
| \|\|q\|^2 q\|               | 11.376  | 26.688  | 50.610   |
| \|\|q\|^2 q\|/\|iq_t+q_xx\| | 0.554   | 0.591   | 0.614    |
...
\begin{table}[!ht]
    ...
    \begin{tabular}{|l|l|l|l|}
    \hline
        ~ & mu=2.30 & mu=4.15 & mu=6.50  \\ \hline
        |iq\_t + q\_xx| & 20,521 & 45,121 & 82,383  \\ \hline
        ||q|\^2 q| & 11,376 & 26,688 & 50,610  \\ \hline
        ||q|\^2 q|/|iq\_t+q\_xx| & 0,554 & 0,591 & 0,614  \\ \hline
    \end{tabular}
\end{table}
-->
Нелинейность уравнения также растёт, поэтому с ростом $\mu$ решение всё сильнее отличается от решения линейного уравнения и всё сложнее найти его. Если $alpha>0$, то рост нелинейности с увеличением $\mu$ будет более существенным и поэтому зависимость точности решения от $\mu$ по идее будет сильнее.  

Дополнительно можно рассмотреть поведение величин $L_1$ и $L_2$(физический смысл которых характеризует энергию системы) при увеличении $\mu$. Они вычисляются по соответствующим формулам: 
...
Зависимость не является линейной, ведь $k$ и $w$ встречаются в формуле решения не только в составе $\mu$. Тем не менее, явно видно, что при увеличении $\mu$ рассматриваемые величины также растут. Можно даже заметить сходство уменьшения точности $mse_q$ на графике 5 и увеличение $L_1$ и $L_2$ на графике 7.
...
### Вывод
В данной работе изучалась зависимость точности получаемого с помощью PINN решения от коэффициентов $k$ и $w$ в начальном условии. Было получено, что при увеличении $k$ и уменьшении $w$ решение становится сложнее аппроксимировать, слагаемые минимизируемых выражений растут, а нелинейность уравнения увеличивается, из-за чего точность решения ожидаемо снижается.  
# Обзор Литературы  
